...
dfhipoteca<- rbind(ecv06 %>% 
  filter(emancipado==1) %>% 
  mutate(hipoteca= ifelse(is.na(alquiler), gastovivienda, 0)) %>% 
  group_by(grupoedad) %>% 
  summarise(año="2006",
            mean=weighted.mean(hipoteca, pesos, na.rm=T)),
ecv18 %>% 
  filter(emancipado==1) %>% 
  mutate(hipoteca= ifelse(is.na(alquiler), gastovivienda, 0)) %>% 
  group_by(grupoedad) %>% 
...
ecv18 %>% 
  filter(emancipado==1, is.na(alquiler)) %>% 
  count(alquiler)

dfhipalquiler<- rbind(ecv06 %>% 
                     filter(emancipado==1) %>% 
                     mutate(alqui= ifelse(alquiler>0, gastovivienda, 0)) %>% 
                     group_by(grupoedad) %>% 
                     summarise(año="2006",
                               mean=weighted.mean(alqui, pesos, na.rm=T)),
                   ecv18 %>% 
                     filter(emancipado==1) %>% 
                     mutate(alqui= ifelse(alquiler>0, gastovivienda, 0)) %>% 
                     group_by(grupoedad) %>% 
